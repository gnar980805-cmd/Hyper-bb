function [sigma,data]= get_hyperbb_sigmacorr_v2(wvs,cp,ap,bbr,env,instr)
% get_hyperbb_sigmacorr_v2 - Computing sigma correction factor accounting
% for attenuation of light and potential multiple scattering
% Usage:
%   1. sigma = get_hyperbb_sigmacorr_2026(wvs, cp, ap，bbr, env)
%   2. sigma = get_hyperbb_sigmacorr_2026(wvs, cp, ap，bbr, env, instr)
%   Input Arguments
%       wvs     - wavelength [nm]
%       cp      - Non-water attenuation coefficient [m-1]
%       ap      - Non-water absorption coefficient [m-1]
...
% 
%       instr.c_type = 'ACS';
%       instr.c_type = 'LISST-VSF';
%   OR alternatively input acceptance angle
%       instr.acc_ang = [0.9]; % ACS
%
%   Required Functions: get_Water_Absorption_Coef, betasw_HZP2019, get_ACS_TS_Correction
...
if nargin == 6
    if isfield(instr,'c_type') % attenuation meter type
        ...
        elseif strcmpi(instr.c_type,'LISST-VSF') || strcmpi(instr.c_type,'LISSTVSF')
            acc_angle = 0.0387; % acceptance angle
        else
    ...
    [corr_bbr,~]= bbr_sca_corr(bbr,nan,acc_angle,ang_a);
end
...
rf_a = get_rf(bbr,ang_a); % if scattering correction is not conducted
ap_corr = ap - (1-rf_a)./(rf_a-rf_cmeter).*bp; % true a
bp_corr = 1./(rf_a-rf_cmeter).*bp;% true b
...
c_true = ap_corr + bp_corr; % true c
c_hyperbb = ap_corr + (1-rf_hyperbb).*bp_corr;  % equivalent c of hyper-bb
sigma = get_sigma(cwi,c_hyperbb+cw); % use total c
% supplementary data
data.bbr_true = bbr;
data.ssa_true = bp_corr./c_true./(1-rf_cmeter);
data.cp_true = c_true; 
data.rf_hyper = rf_hyperbb;
data.rf_cmeter = rf_cmeter;
data.ap_true = ap_corr;
data.bp_true = bp_corr;
end
